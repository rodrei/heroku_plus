...
gem also makes available all default Heroku commands enhanced with your current account settings. Once you get your feet wet this this gem, you'll
soon find that you'll be using less of the _heroku_ command and more of the _hp_ command.

Thanks to Ruby5[http://ruby5.envylabs.com] for mentioning this gem in {Episode 103}[http://ruby5.envylabs.com/episodes/105-episode-103-august-17-2010].
...
From a command line, type: hp -h

Full usage is displayed below:

Account Management:
  -s, --switch_account ACCOUNT     Switch Heroku credentials and SSH identity for specified account.
  -B, --backup_account ACCOUNT     Backup existing Heroku credentials and SSH identity for specified account.
  -D, --destroy_account ACCOUNT    Destroy Heroku credentials and SSH identity for specified account.
  -M, --mode MODE                  Switch mode for current account.

Heroku Commands:
  -p, --pass COMMAND               Pass command to Heroku for current app. TIP: Wrap complex commands in quotes.
  -c, --console                    Open remote console for current app.
  -m, --migrate                    Migrate remote database and restart server for current app.
  -r, --restart                    Restart remote server for current app.
  -b, --backup                     Backup PostgreSQL database on remote server for current app.
  -I, --import RAILS_ENV           Import latest remote PostgreSQL database into local database for given environment for current app.

General Information:
  -i, --info                       Show current Heroku credentials and SSH identity.
