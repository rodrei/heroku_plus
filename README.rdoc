...
From the command line, type: hp
  ...
  hp account       # Show account information.
  hp pass COMMAND  # Pass command to Heroku.
  hp console       # Open remote console.
  hp restart       # Restart remote server.
  hp migrate       # Migrate remote database and restart server.
  hp db            # Manage PostgreSQL database.
  hp version       # Show version.
  hp help          # Show this message.

For more account options, type: hp help account

  -l, [--list = Show all configured accounts]               
  -s, [--switch = Switch to existing account]               
  -m, [--mode = Switch account mode]                        
  -d, [--destroy = Delete existing account]                 
  -b, [--backup = Backup existing account to new account]   
...
For more db options, type: hp help db
  ...
  -i, [--import = Import latest remote PostgreSQL database into local database.] # Default: development
